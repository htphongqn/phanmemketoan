...
{
    public class KTGTCTRepo
    {
        dbVstoreAppDataContext db = new dbVstoreAppDataContext();

        public virtual KT_GTCT GetById(int id)
        {
            try
            {
                return this.db.KT_GTCTs.Single(u => u.ID == id);
            }
            catch
            {
                return null;
